...
export default function DiabetesWideCard({ onPress }: Props) {
  ...
  const handlePress = () => {
    if (onPress) {
      ...
      router.push('/packages/category/diabetes' as any);
    }
  };
      ...
      >
        <View style={styles.content}>
          <View style={styles.badgeRow}>
            <Sparkles size={11} color="#F57F17" />
            <Text style={styles.badgeText}>FREE HBA1C TEST</Text>
          </View>
          <Text style={styles.title} numberOfLines={1}>Diabetes Care Card</Text>
          <Text style={styles.subtitle} numberOfLines={2}>Continuous glucose monitoring & organ protection</Text>
        </View>
        
        <View style={styles.imageContainer}>
          <LinearGradient 
            colors={['#FFF8E1', 'transparent']} 
            start={{ x: 0, y: 0 }} 
            end={{ x: 0.6, y: 0 }} 
            style={styles.imageGradientOverlay} 
          />
          <Image
            source={{ uri: 'https://images.unsplash.com/photo-1584515979956-d9f6e5d09982?q=80&w=600' }}
            style={styles.image}
            contentFit="cover"
          />
        </View>
