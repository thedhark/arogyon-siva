...
import { AnalyticsOverviewCard } from '../../../components/partner/AnalyticsOverviewCard';
import { PostPublisherCard } from '../../../components/partner/PostPublisherCard';
...
export default function PartnerDashboardScreen() {
  ...
  const { profile, leads, posts, boostTiers, activeBoostId } = usePartnerStore();
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      <View style={[styles.navBar, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        ...
        <Pressable onPress={() => router.push('/(partner)/content' as any)} style={styles.navItem}>
          <Ionicons name="newspaper-outline" size={16} color={colors.text} />
          <Text style={[styles.navText, { color: colors.text }]}>Content</Text>
        </Pressable>
        <Pressable onPress={() => router.push('/(partner)/boost' as any)} style={styles.navItem}>
          <Ionicons name="flash-outline" size={16} color="#EC4899" />
          <Text style={[styles.navText, { color: colors.text }]}>Boost</Text>
        </Pressable>
        <Pressable onPress={() => router.push('/(partner)/leads' as any)} style={styles.navItem}>
          <Ionicons name="people-outline" size={16} color={colors.text} />
          <Text style={[styles.navText, { color: colors.text }]}>Leads</Text>
        </Pressable>
        <Pressable onPress={() => router.push('/(partner)/listings' as any)} style={styles.navItem}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        {activeBoost && (
          <Pressable
            onPress={() => router.push('/(partner)/boost' as any)}
            style={({ pressed }) => [
              styles.boostBanner,
        ...
        <View style={styles.sectionHeaderRow}>
          ...
          <Pressable onPress={() => router.push('/(partner)/leads' as any)}>
        ...
        <View style={styles.sectionHeaderRow}>
          ...
          <Pressable onPress={() => router.push('/(partner)/content' as any)}>
